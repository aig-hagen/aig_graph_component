...
const isSubmittable = computed(
    () =>
        (tab.value === 0 &&
            fileInput?.value &&
            ((fileInput?.value as unknown as File).name.toLowerCase().endsWith('.tgf') ||
                (fileInput?.value as unknown as File).name.toLowerCase().endsWith('.json'))) ||
        (tab.value === 1 && props.graphAsTgf !== '')
)

const fileInputRules = [
    (value: File | undefined) => !!value || 'File is required',
    (value: File | undefined) =>
        !value ||
        /\.(tgf|TGF|json|JSON)$/.test(value.name) ||
        ...
        navigator.clipboard
            .writeText(
                exportFormat.value === 'TGF'
                    ...
                    : JSON.stringify(props.graphAsJson, null, 4)
            )
            .then(
